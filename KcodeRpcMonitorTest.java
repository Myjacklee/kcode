...
public class KcodeRpcMonitorTest {
    public static void main(String[] args) throws Exception {
        ...
        kcodeRpcMonitor.prepare("H:\\data\\input\\kcodeRpcMonitor\\2kcodeRpcMonitor.data");
        System.out.println("prepare 耗时(ms):" + NANOSECONDS.toMillis(nanoTime() - startNs));

        // 读取checkPair.result文件
        Map<CheckPairKey, Set<CheckPairResult>> checkPairMap = createCheckPairMap("H:\\data\\output\\checkPair.result");

        // 读取checkResponder.result文件
        Map<CheckResponderKey, CheckResponderResult> checkResponderMap = createCheckResponderMap("H:\\data\\output\\checkResponder.result");
